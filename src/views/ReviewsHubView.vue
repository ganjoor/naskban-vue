...
// Every published review across every book, paginated - the site-wide
// counterpart to PDFBookReviewsView.vue, mirroring CommentsHubView.vue's
// own structure with the vote/edit/delete/write-dialog logic from
// PDFBookReviewsView.vue layered in (no per-book rating summary or sort
// selector here, since this spans every book at once).
...
async function loadReviews() {
  loading.value = true
  const res = await PDFBookReviewService.getRecentReviews(userInfo.value, pageNumber.value)
  if (res) {
...
onMounted(() => {
  ...
  document.title = 'نسکبان - نقدهای کاربران'
  loadReviews()
})
...
<template>
  ...
  <div class="row justify-center items-center q-pa-sm">
    <h3 class="q-my-none">نقدهای کاربران</h3>
  </div>
